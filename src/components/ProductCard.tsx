...
export function ProductCard({product,index=0}:{product:Product;index?:number}){
  ...
  return <article className={'card '+(index%2?'offset':'')} data-reveal="card" style={{'--i':index%4} as React.CSSProperties}>
    ...
    <div className="meta"><div><Link to={'/product/'+product.slug} data-cursor="VER"><h3>{product.name}</h3></Link><p>{product.subtitle}</p></div><b>{money(product.price)}</b></div>
