...
export function CartDrawer(){
  ...
  return <><div className="backdrop" onClick={ui.closeCart}></div><aside className="cart" aria-label="Carrito" aria-hidden={!ui.cartOpen}>
    ...
    <div className="cart-items">{commerce.lines.length?commerce.lines.map(line=><article className="cart-item" key={line.key}><ImageWithFallback src={line.product.media[0]?.publicUrl} alt={line.product.name}/><div className="cart-item-info"><b>{line.product.name}</b><span>TALLA / {line.variant.size}</span><strong>{money(line.variant.price??line.product.price)}</strong><div className="quantity"><button onClick={()=>commerce.updateLine(line.key,line.quantity-1)}>−</button><span>{line.quantity}</span><button onClick={()=>commerce.updateLine(line.key,line.quantity+1)}>+</button></div><button className="remove" onClick={()=>commerce.removeLine(line.key)}>ELIMINAR</button></div></article>):<div className="empty-cart"><span>TU CARRITO ESTÁ VACÍO.</span><Link to="/shop" onClick={ui.closeCart}>VER LA TIENDA ↗</Link></div>}</div>
