...
import { displayCategory, normalizeLegacyProduct } from '../lib/clientContent'
...
export function AdminProductEditPage(){
  ...
  async function load(){
    try{
      ...
      else{const found=c.products.find(x=>x.id===id);if(!found)throw new Error('Producto no encontrado');const p=normalizeLegacyProduct(found);const cols=p.collections.map(x=>x.id);setProduct(p);setCollections(cols);setBaseline(JSON.stringify({product:p,collections:cols}))}
      setError(null)
  ...
  const isPerfume=(catalog.categories.find(c=>c.id===product.categoryId)?.name||product.category)==='Perfumes'
  return <div className="admin-page admin-editor">
    <header className="admin-page-head"><div><span>PRODUCTO / {isNew?'NUEVO':product.name}</span><h1>{isNew?'CREAR.':'EDITAR.'}</h1><p>Edita nombre, descripción, categoría, estilo/subtipo, precios, composición, variantes e imágenes.</p></div><button className="btn dark" disabled={!dirty||saveState==='saving'} onClick={save}>{saveState==='saving'?'GUARDANDO…':saveState==='saved'?'GUARDADO ✓':'GUARDAR'}</button></header>
    {error&&<div className="admin-warning">{error}</div>}
    <section className="admin-form-grid"><label>Nombre<input value={product.name} onChange={e=>patch('name',e.target.value)}/></label><label>Slug<input value={product.slug} onChange={e=>patch('slug',e.target.value)}/></label><label>Subtítulo<input value={product.subtitle||''} onChange={e=>patch('subtitle',e.target.value)}/></label><label>Categoría<select value={product.categoryId||''} onChange={e=>patch('categoryId',e.target.value||null)}><option value="">Sin categoría</option>{catalog.categories.map(c=><option key={c.id} value={c.id}>{displayCategory(c.name)}</option>)}</select></label><label>Estilo / subtipo<select value={product.fit||''} onChange={e=>patch('fit',e.target.value||null)}><option value="">Por confirmar</option>{catalog.fits.map(f=><option key={f.id} value={f.name}>{f.name}</option>)}</select></label><label>Color<input value={product.color||''} onChange={e=>patch('color',e.target.value||null)}/></label>{isPerfume&&<label>Familia olfativa<input value={product.fragranceFamily||''} onChange={e=>patch('fragranceFamily',e.target.value||null)} placeholder="Ej. Ámbar floral"/></label>}<label>Precio<input type="number" value={product.price??''} onChange={e=>patch('price',e.target.value?Number(e.target.value):null)}/></label><label>Precio anterior<input type="number" value={product.compareAtPrice??''} onChange={e=>patch('compareAtPrice',e.target.value?Number(e.target.value):null)}/></label><label>Estado<select value={product.status} onChange={e=>patch('status',e.target.value as Product['status'])}><option value="draft">Borrador</option><option value="active">Activo</option><option value="hidden">Oculto</option><option value="archived">Archivado</option></select></label><label>Orden<input type="number" value={product.sortOrder} onChange={e=>patch('sortOrder',Number(e.target.value))}/></label><label className="wide">Descripción breve<textarea rows={3} value={product.description||''} onChange={e=>patch('description',e.target.value)}/></label><label className="wide">{product.category==='Perfumes'?'Composición / notas principales':'Detalles adicionales'}<textarea rows={4} value={product.features.join('\n')} onChange={e=>patch('features',e.target.value.split('\n').map(x=>x.trim()).filter(Boolean))} placeholder={product.category==='Perfumes'?'Una nota o dato por línea. No cargar información no confirmada.':'Un detalle por línea'}/></label></section>
    ...
    <section className="admin-section"><div className="section-head"><h2>VARIANTES</h2><button onClick={addVariant}>+ VARIANTE</button></div><div className="variant-editor">{product.variants.map((v,i)=><div key={v.id}><input placeholder="Talla" aria-label="Talla" value={v.size} onChange={e=>patchVariant(i,'size',e.target.value)}/><input placeholder="Color" aria-label="Color" value={v.color||''} onChange={e=>patchVariant(i,'color',e.target.value||null)}/><input placeholder="SKU" value={v.sku||''} onChange={e=>patchVariant(i,'sku',e.target.value||null)}/><input type="number" placeholder="Precio" value={v.price??''} onChange={e=>patchVariant(i,'price',e.target.value?Number(e.target.value):null)}/><input type="number" placeholder="Stock" value={v.stock??''} onChange={e=>patchVariant(i,'stock',e.target.value?Number(e.target.value):null)}/><label><input type="checkbox" checked={v.available} onChange={e=>patchVariant(i,'available',e.target.checked)}/> Disponible</label><button onClick={()=>removeVariant(i)}>ELIMINAR</button></div>)}</div></section>
